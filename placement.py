...
def get_args():
    parser = argparse.ArgumentParser(
        description='Calculate distance from 2 lists of genomes using mash'
    )
    parser.add_argument('genomes_list_1', help='path to the first list')
    parser.add_argument('genomes_list_2', help='path to the second list')
    ...
    parser.add_argument('--statistic_file_type', type=str, default='json', help='Output json or csv')
...
def placement():
    ...
    stats = {
        ...
        "timings": {},
        "resources": {"max_rss_MB": round(usage.ru_maxrss / 1024, 2)}
    }
    ...
    if args.statistic:
        if args.statistic_file_type == 'json':
            stats_path = os.path.join(args.output, 'placement_stats.json')
            with open(stats_path, 'w') as f:
                json.dump(stats, f, indent=2)
            if args.verbose:
                print(f"Statistics saved to: {stats_path}")
        elif args.statistic_file_type == 'csv':
            stats_file = os.path.join(args.output, f"placement_stats.csv")
